 ...
 * @param c - The standard deviation (or Gaussian RMS width),
 *  which controls the width of the bell
 * @throws {@link DivideByZeroError} if `c` is equal to 0
 */
export function gaussian(x: number, a: number, b: number, c: number): number {
	if(c === 0) { throw new RangeError('c must be a non-zero number'); }
